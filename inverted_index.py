...
def inverted_index(): 
  ...
  # start getting all files in the corpus 
  for root, dirs, files in os.walk("./DEV"):
    for doc in files:
      ...
      with open(doc_name, "r", encoding='utf-8', errors='replace') as opened:
        ...
        url = json_fields['url'] 
        url = url.split('#')[0]
        # check if the url is seen (may be duplicates because urls because of frags) 
        if url not in doc_seen:
          ...
          # find all important text in the document
          temp_imp_index = important_text(parsed_file) # returns { important_text: score }
          # save the important text into another inverted index
          ...
          for token in tokens:
            # only add alphanumeric words to index 
            if token.isalnum(): 
              stem_word = porter.stem(token)
              # if the word is not in the index, add the doc_id + initial tf 
              if stem_word not in index_dict:
                index_dict[stem_word][doc_id] = 1
              # Token is already in index, but we need to add NEW doc_id + inital tf 
...
# creates an inverted index with tf-idf scores
  ...
  with open("new_inverted_index.txt", "w") as new_index_file:
    with open(file_name, "r") as inverted_index_file:
      ...
      for line in inverted_index_file:
        ...
        transfer_posting = (posting[0], tfidf_index) # save as a set to store and write to a new inverted_index file
...
if __name__ == "__main__":
  ...
  # Bookkeeping file to map the doc_ids to urls
  ...
  # Using the merged_index, create an inverted index that now contains TF-IDF scores
  ...
  # create an index of the important text inverted index for seek() 
